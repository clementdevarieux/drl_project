...
pub fn greedy_action<B: Backend<FloatElem=f32, IntElem=i64>, const NUM_STATES_FEATURES: usize, const NUM_ACTIONS: usize>(
    q_s: &Tensor<B, 1>,
    ...
    for ep_id in tqdm!(0..num_episodes) {
        ...
        if ep_id % 1000 == 0 {
            ...
            for _ in 0..1000 {
                ...
                while !env.is_game_over() {
                    let s = env.state_description();
                    let s_tensor: Tensor<B::InnerBackend, 1> = Tensor::from_floats(s.as_slice(), device);

                    let mask = env.action_mask();
                    let mask_tensor: Tensor<B::InnerBackend, 1> = Tensor::from(mask).to_device(device);
        ...
        while !env.is_game_over() {
            ...
            let (_, q_s_p_a_p) = if env.is_game_over() {
                (0, Tensor::from([0f32]).to_device(device))
            } else {
                let a_p = greedy_action::<B, NUM_STATE_FEATURES, NUM_ACTIONS>(
                    &q_s_p,
                    &mask_p_tensor
                );
                #[allow(clippy::single_range_in_vec_init)]
